...
+(FMDatabase *)ShareDataBase
{
    ...
    NSString * createTB = @"create table orderMenu(proID int,Menuid int,proName nvarchar(50),price double,proImage varchar(50),number double,typeID int,typeName nvarchar(50))";
...
+(void)UpdateDotNumber:(int)aProid currDotNumber:(double)aDotNumber
{
    ...
    [db open];
    [db beginTransaction];
    NSString * sql = [NSString stringWithFormat:@"update orderMenu set number='%g' where proID = '%d'",aDotNumber,aProid];
    BOOL isSuccess = [db executeUpdate:sql];
    [db commit];
    if (isSuccess)
    {
...
+(void)insertProID:(int)aProID menuid:(int)aMenuId proName:(NSString *)aName price:(double)aPrice image:(NSString *)aImage andNumber:(double)aNumber typeID:(int)aTypeID typeName:(NSString *)aTypeName
{
    ...
    [db open];
    [db beginTransaction];
    NSString * sql = [NSString stringWithFormat:@"insert into orderMenu(proID,Menuid,proName,price,proImage,number,typeID,typeName) values('%d','%d','%@','%g','%@','%g','%d','%@')",aProID,aMenuId,aName,aPrice,aImage,aNumber,aTypeID,aTypeName];
    BOOL isSuccess = [db executeUpdate:sql];
    [db commit];
    if (isSuccess)
    {
...
+(NSString *)SelectTypeIDByTypeName:(NSString *)aTypeName
{
    FMDatabase * db = [DataBase ShareDataBase];
    [db open];
    NSString * sql = [NSString stringWithFormat:@"select typeID from orderMenu where typeName='%@'",aTypeName];
    FMResultSet * rs = [db executeQuery:sql];
    NSString * typeID;
    while([rs next])
    {
